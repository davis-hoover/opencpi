 ...
 */
#include "complex_mixer_tutorial-worker.hh"
...
using namespace Complex_mixer_tutorialWorkerTypes;
using namespace std;

#define Uscale(x)  (float)((float)(x) / (pow(2,15) -1))
#define Scale(x)   (int16_t)((float)(x) * (pow(2,15) -1))

class Complex_mixer_tutorialWorker : public Complex_mixer_tutorialWorkerBase
{
  nco_crcf q;
...
COMPLEX_MIXER_TUTORIAL_START_INFO
// Insert any static info assignments here (memSize, memSizes, portInfo)
// e.g.: info.memSize = sizeof(MyMemoryStruct);
COMPLEX_MIXER_TUTORIAL_END_INFO
