...
# "bash" is required: "/bin/sh" points to "dash" on Mint 21.1 systems.
#
##########################################################################################
# Install or list required and available packages for Mint 21.1
#
...
# Add 32-bit cross-architecture packages as required: Mint has multi-arch support
...
#    for CentOS7, "which" is in a separate package: Mint has it in debianutils
PKGS_D+=(debianutils)
#    for development and solving the "/lib/cpp failed the sanity check" a long shot
#    Mint library packages include the static versions
...
#    for Mint 21.1, installing "lsb" covers most of what we need
...
#    N.B.: "libpng12-0" is not available on Mint 21.1.  Will substitute
...
#    Needed for processing Xilinx release tarball(s).
PKGS_D+=(fakeroot)
...
#    required version is 3.6 these days.  For Mint 21.1, "python3" maps to "python3.10".
#
#    for creating swig
