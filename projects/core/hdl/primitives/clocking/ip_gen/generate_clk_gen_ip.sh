...
if [[ "$Target" == "zynq" || "$Target" == "zynq_ultra" ]]; then
  ...
  for fileset in $AlteraFileSets; do 
    ...
    cd tmp &&  $VendorDir/sopc_builder/bin/ip-generate --verbose --language=vhdl --output-directory=. --file-set=$fileset \
    ...
    find . -name $origFileName -exec mv {} ../$newFileName \; && cd ../ && patch -F 5 < $patchName && rm -f ${AlteraPllName}.vhd.orig && mv $newFileName $destinationDir && cd ../;
  done
