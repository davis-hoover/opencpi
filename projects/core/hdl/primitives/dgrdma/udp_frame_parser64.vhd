...
-- The output axis bus of this component is 64-bit wide.
-- ---------------------------------------------------------------------------
library ieee; use ieee.std_logic_1164.all; use ieee.numeric_std.all;
use work.dgrdma_util.all;

entity udp_frame_parser64 is
  generic (
...
architecture rtl of udp_frame_parser64 is

-- FSM state
type state_t is (S_IDLE, S_HEADER, S_PAYLOAD, S_TLAST);
...
signal last_payload_tdata : std_logic_vector(DATA_WIDTH-1 downto 0);
signal last_payload_tkeep : std_logic_vector(KEEP_WIDTH-1 downto 0);
...
begin
  ...
  -- other than IDLE and LAST
  with state select s_udp_payload_axis_tready_r <=
    '0' when S_IDLE | S_TLAST,
  ...
  begin
    if rising_edge(clk) then
      ...
      if reset = '1' then
        ...
        last_payload_tdata   <= (others => '0');
        last_payload_tkeep   <= (others => '0');
        rx_match             <= false;
        ...
        case state is
          ...
          -- output the REMOTE IP address and UDP Port (48-bits)
          -- and 16-bits of UDP payload (note first two bytes
          -- of the payload in DG-RDMA packets is padding)
          when S_HEADER =>
            if s_axis_tready_r = '1' then
              udp_dest_port      <= remote_udp_dest_port;
              m_rx_axis_tdata    <= s_udp_payload_axis_tdata(31 downto 16) & remote_udp_src_port & remote_ip_addr;
              m_rx_axis_tkeep    <= s_udp_payload_axis_tkeep(3 downto 2) & b"111111";
              m_rx_axis_tlast    <= '0';
              ...
              if s_udp_payload_axis_tvalid = '1' then
                ...
                last_payload_tdata <= s_udp_payload_axis_tdata;
                last_payload_tkeep <= s_udp_payload_axis_tkeep;

                -- check if this is the last of the payload
                if s_udp_payload_axis_tlast = '1' then
                  if s_udp_payload_axis_tkeep(7 downto 4) = b"0000" then
          ...
          when S_PAYLOAD =>
            if s_axis_tready_r = '1' then
              m_rx_axis_tdata    <= s_udp_payload_axis_tdata(31 downto 0) & last_payload_tdata(63 downto 32);
              m_rx_axis_tkeep    <= s_udp_payload_axis_tkeep(3 downto 0) & last_payload_tkeep(7 downto 4);
              m_rx_axis_tlast    <= '0';
              m_rx_axis_tvalid_r <= s_udp_payload_axis_tvalid;
          ...
          when S_TLAST =>
            if s_axis_tready_r = '1' then
              m_rx_axis_tdata    <= X"00000000" & last_payload_tdata(63 downto 32);
              m_rx_axis_tkeep    <= b"0000" & last_payload_tkeep(7 downto 4);
              m_rx_axis_tlast    <= '1';
