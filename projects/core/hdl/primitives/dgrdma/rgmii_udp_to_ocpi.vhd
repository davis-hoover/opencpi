...
end entity rgmii_udp_to_ocpi;

architecture rtl of rgmii_udp_to_ocpi is

  -- The width of the AXI-Stream from/to ethernet MAC
  -- UDP uses 8-bit MAC when operating with SDP_WIDTH=1
  -- and 64-bit MAC for SDP_WIDTH=2
  function ethernet_data_width(specified_sdp_width: integer) return integer is
  begin
    if (specified_sdp_width = 1) then
      return(8);
    else
      return(specified_sdp_width*32);
    end if;
  end function ethernet_data_width;

  function ethernet_keep_width(specified_sdp_width: integer) return integer is
  begin
    if (specified_sdp_width = 1) then
      return(1);
    else
      return(specified_sdp_width*4);
    end if;
  end function ethernet_keep_width;

  constant DATA_WIDTH  : integer := ethernet_data_width(SDP_WIDTH);
  constant KEEP_WIDTH  : integer := ethernet_keep_width(SDP_WIDTH);
...
begin
  ...
  udp_to_ocpi_inst : entity work.udp_to_ocpi
    generic map (
      SDP_WIDTH                     => SDP_WIDTH,
      ETH_DATA_WIDTH                => DATA_WIDTH,
      ETH_KEEP_WIDTH                => KEEP_WIDTH,
      ...
      MAX_FRAME_SIZE                => MAX_FRAME_SIZE,
      UDP_CP_PORT                   => UDP_CP_PORT,
      UDP_SDP_PORT                  => UDP_SDP_PORT
    )
    port map (
      clk       => clk,
      reset     => reset,
