...
template<class log_t,class cfgrtr_t> uint8_t
AD9361DRC<log_t,cfgrtr_t>::get_ch(data_stream_id_t id) {
  uint8_t res = 
      (id == m_ds_rx1) ? RX1 :
...
template<class log_t,class cfgrtr_t> bool
AD9361DRC<log_t,cfgrtr_t>::get_enabled(data_stream_id_t id) {
  bool ret = true;
  get_rx_and_throw_if_invalid_ds(id);
#ifndef DISABLE_AD9361
  if(not m_ad9361_init_called) {
    ret = false;
  }
...
template<class log_t,class cfgrtr_t> uint64_t
AD9361DRC<log_t,cfgrtr_t>::get_noos_tuning_freq(data_stream_id_t id) {
  init_if_required();
  uint64_t ret = 0;
...
template<class log_t,class cfgrtr_t> uint32_t
AD9361DRC<log_t,cfgrtr_t>::get_noos_bandwidth(data_stream_id_t id) {
  init_if_required();
  uint32_t ret = 0;
  bool do_rx = get_rx_and_throw_if_invalid_ds(id);
  do_rx ?
...
template<class log_t,class cfgrtr_t> uint32_t
AD9361DRC<log_t,cfgrtr_t>::get_noos_sampling_rate(data_stream_id_t id) {
  init_if_required();
  uint32_t ret = 0;
  bool do_rx = get_rx_and_throw_if_invalid_ds(id);
  do_rx ?
...
template<class log_t,class cfgrtr_t> uint8_t
AD9361DRC<log_t,cfgrtr_t>::get_noos_gain_mode(data_stream_id_t id) {
  init_if_required();
...
template<class log_t,class cfgrtr_t> int32_t
AD9361DRC<log_t,cfgrtr_t>::get_noos_rx_gain(data_stream_id_t id) {
  init_if_required();
  int32_t ret = 0;
  if((id != m_ds_rx1) && (id != m_ds_rx2)) { 
    throw std::string("invalid rx data stream id ") + id;
  }
...
template<class log_t,class cfgrtr_t> uint32_t
AD9361DRC<log_t,cfgrtr_t>::get_noos_tx_gain(data_stream_id_t id) {
  init_if_required();
  uint32_t ret = 0;
  if((id != m_ds_tx1) && (id != m_ds_tx2)) { 
    throw std::string("invalid tx data stream id ") + id;
  }
...
AD9361DRC<log_t,cfgrtr_t>::get_direction(data_stream_id_t id) {
  bool do_rx = get_rx_and_throw_if_invalid_ds(id);
...
AD9361DRC<log_t,cfgrtr_t>::get_tuning_freq_MHz(
    data_stream_id_t id) {
  return ((config_value_t)get_noos_tuning_freq(id))/1000000.;
}
...
AD9361DRC<log_t,cfgrtr_t>::get_bandwidth_3dB_MHz(
  ...
  config_value_t ret = (config_value_t)get_noos_bandwidth(id);
...
AD9361DRC<log_t,cfgrtr_t>::get_sampling_rate_Msps(
  ...
  ///@TODO / FIXME handle DDC/DUC divider in separate DDC/DUC class
  config_value_t ret = (config_value_t)get_noos_sampling_rate(data_stream_id);
  ret = ret/1000000./m_divider;
  return ret;
}
...
AD9361DRC<log_t,cfgrtr_t>::get_gain_mode(data_stream_id_t id) {
  std::string ret = "manual";
#ifndef DISABLE_AD9361
  uint8_t val = get_noos_gain_mode(id);
  if(val == RF_GAIN_MGC) {
    ret = "manual";
  }
...
template<class log_t,class cfgrtr_t> config_value_t 
AD9361DRC<log_t,cfgrtr_t>::get_gain_dB(data_stream_id_t id) {
  bool do_rx = get_rx_and_throw_if_invalid_ds(id);
  if(do_rx) {
    return (config_value_t)get_noos_rx_gain(id);
  }
  else {
    return (config_value_t)convert_milli_db_to_db(get_noos_tx_gain(id));
  }
}

template<class log_t,class cfgrtr_t> void
AD9361DRC<log_t,cfgrtr_t>::set_direction(data_stream_id_t id,
...
AD9361DRC<log_t,cfgrtr_t>::set_tuning_freq_MHz(
    data_stream_id_t id,config_value_t val) {
  init_if_required();
...
template<class log_t,class cfgrtr_t> void
AD9361DRC<log_t,cfgrtr_t>::set_bandwidth_3dB_MHz(data_stream_id_t id,
    config_value_t val) {
  init_if_required();
  ///@TODO / FIXME handle DDC/DUC divider in separate DDC/DUC class
...
AD9361DRC<log_t,cfgrtr_t>::set_gain_mode(
    data_stream_id_t id, gain_mode_value_t val) {
  init_if_required();
  bool do_rx = get_rx_and_throw_if_invalid_ds(id);
...
//template<class log_t,class cfgrtr_t> bool
//AD9361DRC<log_t,cfgrtr_t>::start(unsigned config) {
//}

template<class log_t,class cfgrtr_t> bool
AD9361DRC<log_t,cfgrtr_t>::stop(unsigned config) {
  bool ret = true;
#ifndef DISABLE_AD9361
  ///@TODO / FIXME - why does this methods set AD9361 to ALERT???
  ///@TODO / FIXME - ********************************************************************************************************************************************************************************************************************************************************************************************************************comment back in!!!!
  //ret = ad9361_set_en_state_machine_mode(m_ad9361_rf_phy, ENSM_MODE_ALERT);
#endif
  if(!DRC<log_t,cfgrtr_t>::stop(config)) {
    ret = false;
  }
  return ret;
}
