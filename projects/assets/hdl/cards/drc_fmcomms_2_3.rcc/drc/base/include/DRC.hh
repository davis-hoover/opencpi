...
class Configurator {
  ...
  const std::vector<data_stream_id_t>& get_data_stream_ids() const;
...
class DataStreamConfigLockRequest {
  protected:
  data_stream_id_t        m_data_stream_id;
  data_stream_direction_t m_direction;
  ...
  data_stream_direction_t get_direction() const;
  data_stream_id_t        get_data_stream_id() const;
   ...
   ******************************************************************************/
  void include_data_stream_direction(data_stream_direction_t val);
  /*! @brief Either this function or include_data_stream_direction()
   *         must be called before sending a
   *         DataStreamConfigLockRequest to ARC::request_config_lock or
   *         DRC::request_config_lock.
   ******************************************************************************/
  void include_data_stream_id(data_stream_id_t val);
   ...
   *                        etc...
   ******************************************************************************/
  void include_routing_id(routing_id_t val);
  /*! @brief This function must be called before sending a
   *         DataStreamConfigLockRequest to ARC::request_config_lock or
   *         DRC::request_config_lock.
   ******************************************************************************/
  void include_tuning_freq_MHz(config_value_t val, config_value_t tolerance);
  /*! @brief This function must be called before sending a
   *         DataStreamConfigLockRequest to ARC::request_config_lock or
   *         DRC::request_config_lock.
   ******************************************************************************/
  void include_bandwidth_3dB_MHz(config_value_t val, config_value_t tolerance);
  /*! @brief This function must be called before sending a
   *         DataStreamConfigLockRequest to DRC::request_config_lock.
   ******************************************************************************/
  void include_sampling_rate_Msps(config_value_t val, config_value_t tolerance);
  /*! @brief This function must be called before sending a
   *         DataStreamConfigLockRequest to DRC::request_config_lock.
   ******************************************************************************/
   ...
   ******************************************************************************/
  void include_gain_mode(gain_mode_value_t val);
  /*! @brief This function is optionally called before sending a
   *         DataStreamConfigLockRequest to  ARC::request_config_lock or
   *         DRC::request_config_lock. Call this only if request should
   *         include desired manual gain value.
   ******************************************************************************/
  void include_gain_dB(config_value_t val, config_value_t tolerance);
  protected:
...
struct DataStreamConfigLock {
  ...
  data_stream_id_t        m_data_stream_id;
  data_stream_direction_t m_direction;
  /// @todo /FIXME - add this member and implement corresponding functionality?
  //routing_id_t            m_routing_id;
  config_value_t          m_tuning_freq_MHz;
  config_value_t          m_bandwidth_3dB_MHz;
  config_value_t          m_sampling_rate_Msps;
...
class ARC : public LogForwarder<log_t> {
  ...
  std::map<data_stream_id_t,data_stream_direction_t> m_cache_direction;
  std::map<data_stream_id_t,config_value_t>          m_cache_tuning_freq_MHz;
  std::map<data_stream_id_t,config_value_t>          m_cache_bandwidth_3dB_MHz;
  public:
  ...
  virtual bool get_enabled(data_stream_id_t id) = 0;
  virtual data_stream_direction_t get_direction(data_stream_id_t id) = 0;
  virtual config_value_t get_tuning_freq_MHz(data_stream_id_t id) = 0;
  virtual config_value_t get_bandwidth_3dB_MHz(data_stream_id_t id) = 0;
  ...
  virtual void set_direction(data_stream_id_t id,
  ...
  virtual void set_tuning_freq_MHz(
      data_stream_id_t id, config_value_t tuning_freq_MHz) = 0;
  /*! @brief  Attempt to set on-hardware value with no guarantee of success.
   *  @todo / FIXME - for increased functionality, make public and throw
   *          exception if value is locked?
   ******************************************************************************/
  virtual void set_bandwidth_3dB_MHz(
      data_stream_id_t id, config_value_t bandwidth_3dB_MHz) = 0;
  /// @brief Unlocks configurator lock (no hardware action is performed).
  void unlock_tuning_freq_MHz(data_stream_id_t id);
  /// @brief Unlocks configurator lock (no hardware action is performed).
  void unlock_bandwidth_3dB_MHz(data_stream_id_t id);
  /*! @param[in] ds_id Data stream ID for which to apply lock.
   *  @param[in] cfg_key  Config key string.
   ******************************************************************************/
  void unlock_config(data_stream_id_t id, config_key_t cfg_key);
  /*! @brief Performs the minimum config locks required per data stream
   *         for an ARC.
   ******************************************************************************/
  virtual bool do_min_data_stream_config_locks(data_stream_id_t id,
  ...
  public:
  void throw_if_data_stream_disabled_for_read(const data_stream_id_t& id,
      const char* config_description) const;
  void throw_if_data_stream_disabled_for_write(const data_stream_id_t& id,
      const char* config_description) const;
  void throw_invalid_data_stream_id(data_stream_id_t id) const;
}; // class ARC
...
class DRC : public ARC<log_t, configurator_t> {
  protected:
  std::map<data_stream_id_t,config_value_t>    m_cache_sampling_rate_Msps;
  std::map<data_stream_id_t,bool>              m_cache_samples_are_complex;
  std::map<data_stream_id_t,gain_mode_value_t> m_cache_gain_mode;
  std::map<data_stream_id_t,config_value_t>    m_cache_gain_dB;
