...
for msg in msgs:
    ...
    if(msg[utu.MESSAGE_OPCODE] == iqm.SAMPLES_OPCODE):
        ...
        if not msg[utu.MESSAGE_LENGTH] > 0:
            do_fail("Empty message: Invalid message length: SAMPLE msg[%d]" \
              % (num_samples_msgs - 1))
...
if(num_samples_msgs < 1):
    do_fail("at least one SAMPLE messages expected, but received 0")
else:
    do_pass(str(num_samples_msgs) + " SAMPLE messages received")
if(num_time_msgs < 1):
...
if(num_sync_msgs == 1):
    do_pass(str(num_sync_msgs) + " DISCONTINUITY messages received")
else:
    tmp = "exactly one DISCONTINUITY messages expected, but received "
