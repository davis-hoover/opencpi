...
void throw_if_no_data_received_from_adc(OA::Application& app) {
  ...
  if ((!max_i_is_valid) || (!max_q_is_valid))
    throw std::string("no data was received from ADC data path");
  if ((max_i == 0) || (max_q == 0))
...
int main(int argc, char **argv) {
  try {
    ...
    if (app_main_args.do_timeout) {
      throw_if_no_data_received_from_adc(app);
