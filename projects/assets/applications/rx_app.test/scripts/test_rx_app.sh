...
if [ $# -ne 2 ] ; then
   echo "Wrong number of arguments supplied. Syntax is sh `basename $0` <sdr ip address> <sig-gen ip address>"
	...
	if test -n "${USE_EPIQ_APP}"; then
	    ...
	    python3 $directory/verifyRxAppFft.py $directory/$output_file complex 65536 2500000
	else
	    python3 $directory/verifyRxAppFft.py $directory/../../rx_app/odata/rx_app_shortened.out complex 65536 2500000
	fi
	result=$?
	if [ $result -ne 0 ] ; then
	    ((failures++))
    ...
    if test -n "${PLOT}"; then
	python3 $directory/plotFailures.py $log_file &
    fi
fi
