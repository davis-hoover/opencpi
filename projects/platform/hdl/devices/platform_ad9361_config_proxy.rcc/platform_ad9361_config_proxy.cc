...
using namespace Platform_ad9361_config_proxyWorkerTypes;
...
public:
  Platform_ad9361_config_proxyWorker() : m_aRunCondition(RCC_NO_PORTS),
  ...
  ~Platform_ad9361_config_proxyWorker() {
...
#define _2R2Tconfig ((    qadc1_is_present) and (    qdac1_is_present))
  ...
  libad9361_API_chan_get(int32_t function(struct ad9361_rf_phy*,
    ...
    for(uint8_t chan=0; chan<PLATFORM_AD9361_CONFIG_PROXY_RX_NCHANNELS; chan++) {
      libad9361_API_print_idk(functionStr);
  ...
  RCCResult rx_rssi_read() {
    rf_rssi rssi[PLATFORM_AD9361_CONFIG_PROXY_RX_NCHANNELS];
  ...
  RCCResult rx_fir_config_read_read() {
    AD9361_RXFIRConfig cfg[PLATFORM_AD9361_CONFIG_PROXY_RX_NCHANNELS];

    // explicitly copying struct members so as to avoid potential
    // memory corruption due to the fact that OpenCPI sometimes adds
    // additional "padding" members
    for(uint8_t ch=0; ch<PLATFORM_AD9361_CONFIG_PROXY_RX_NCHANNELS; ch++) {
  ...
  */
  ...
  RCCResult tx_fir_config_read_read() {
    AD9361_TXFIRConfig config[PLATFORM_AD9361_CONFIG_PROXY_RX_NCHANNELS];
    RCCResult res = LIBAD9361_API_CHAN_GET(&ad9361_get_tx_fir_config, &(config[0]));
    if(res != RCC_OK) return res;
    // explicitly copying struct members so as to avoid potential
    // memory corruption due to the fact that OpenCPI sometimes adds
    // additional "padding" members
    for(uint8_t chan=0; chan<PLATFORM_AD9361_CONFIG_PROXY_TX_NCHANNELS; chan++) {
...
PLATFORM_AD9361_CONFIG_PROXY_START_INFO
// Insert any static info assignments here (memSize, memSizes, portInfo)
// e.g.: info.memSize = sizeof(MyMemoryStruct);
PLATFORM_AD9361_CONFIG_PROXY_END_INFO
