...
usage(const char *cmd) {
  fprintf(stderr,
	  ...
	  "    bram <infile> <outfile>      # create a BRAM file from input (XML) file\n"
	  ...
	  "    -v                           # be verbose\n"
          "    -P                           # produce parsable output from admin/worker read\n"
  ...
  while (error.empty() && ifs.getNext(eif, error, interface)) {
	...
	if (s.receive(rFrame, length, 0, from, error)) {
	  ...
	  case HE::OCCP_WRITE:
		...
		echp->length = htons((uint16_t)(sizeof(ecrr)-2u));
	      } else {
		...
		echp->length = htons((uint16_t)(sizeof(ecwr)-2u));
	      }
	    ...
	    sent = s.send(out, ntohs(echp->length) + 2u, from, 0, NULL, error);
	    break;
	  case HE::OCCP_NOP:
	    if (OCCP_ETHER_RESERVED(ech_in.typeEtc)) {
	      ...
	      ech_in.length = htons((uint16_t)(sizeof(HE::EtherControlHeader)-2u));
	      ...
	      // Tag is the same
	      ech_in.length = htons((uint16_t)(sizeof(ecnr)-2u));
	    ...
	    sent = s.send(rFrame, ntohs(ech_in.length)+2u, from, 0, NULL, error);
	    break;
	  default:
	    sent = false; // warning
...
ethers(const char **) {
  ...
  while (ifs.getNext(eif, error)) {
    printf("%2d. %s: MAC address %s, %s, %s%s", eif.index,
...
admin(const char **) {
  ...
  time_t gpsNow = (time_t)gpsTimeMS;
  ntime = gmtime(&gpsNow);
...
sendRDMA(const char **ap) {
  ...
  if (*ap) {
    ...
    if (fd < 0 ||
	read(fd, &theInputDesc, sizeof(theInputDesc)) != (ssize_t)sizeof(theInputDesc) ||
	close(fd) < 0)
  ...
  if (*ap) {
    ...
    int fd = creat(file.c_str(), 0666);
    if (fd < 0 ||
	write(fd, &sendDesc, sizeof(sendDesc)) != (ssize_t)sizeof(sendDesc) ||
	close(fd) < 0)
