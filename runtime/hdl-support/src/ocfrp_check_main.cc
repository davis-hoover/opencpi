...
found(const char *name, OU::Bar *bars, unsigned nbars, bool verbose) {
  ...
  do { // break on error to recover mappings
	...
	((uint8_t*)&myUUID)[n] = uuid[(n & ~3u) + (3u - (n&3u))];
      memcpy(platform, myUUID.platform, sizeof(myUUID.platform));
      platform[sizeof(myUUID.platform)] = '\0';
      memcpy(device, myUUID.device, sizeof(myUUID.device));
      device[sizeof(myUUID.device)] = '\0';
      memcpy(load, myUUID.load, sizeof(myUUID.load));
      ...
      bsvbd = (time_t)occp->admin.birthday;
      bsbd = (time_t)myUUID.birthday;
      ctime_r(&bsvbd, tbuf);
