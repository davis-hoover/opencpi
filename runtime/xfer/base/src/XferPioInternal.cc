...
int32_t
xfer_pio_create(EndPoint &src, EndPoint &dst, PIO_template *pio_templatep)
{
  void *src_va;
  void *dst_va;

  // Map source and destination
  src_va = src.sMemServices().mapTx (0, 0);
  dst_va = dst.sMemServices().mapRx (0, 0);
  ...
  pio_template->s_smem = &src.sMemServices();
  pio_template->t_smem = &dst.sMemServices();
...
void
xfer_pio_modify( PIO_transfer transfer, int,  Offset *noff,  Offset *ooff )
{
...
int32_t
xfer_pio_copy(PIO_template pio_template, Offset src_os, Offset dst_os,
