...
class Socket;
class XferServices;
  class DGEndPoint : public OCPI::Xfer::EndPoint, protected virtual OCPI::Util::SelfMutex {
  friend class Socket;
...
protected:
    DGEndPoint(OCPI::Xfer::XferFactory &a_factory, const char *eps, const char *other, bool a_local,
	     size_t a_size, const OCPI::Base::PValue *params)
      : OCPI::Xfer::EndPoint(a_factory, eps, other, a_local, a_size, params), m_xferServices(32) {}
  ~DGEndPoint() {}
...
class XferFactory : public OCPI::Xfer::XferFactory {
  friend class SmemServices;
public:
...
class SmemServices : public OCPI::Xfer::SmemServices, public OCPI::Util::Thread,
...
public:
  SmemServices(DGEndPoint& ep)
    : OCPI::Xfer::SmemServices(ep), m_ep(ep), m_socket(NULL), m_mem(NULL), m_loop(true) {
    if (ep.local()) {
  ...
  void* map (OCPI::Xfer::Offset offset, size_t/* size */) {
    return &m_mem[offset];
  }
...
struct Transaction {
  ...
  void add(uint8_t * src, OCPI::Xfer::Offset dst_offset, size_t length);
...
class XferRequest : public OCPI::Xfer::XferRequest, private Transaction {
public:
  virtual XferServices &parent() = 0;
  XferRequest(XFTemplate *temp)
    : OCPI::Xfer::XferRequest(temp) { //, m_tested4Complete(0) {
  }
  virtual ~XferRequest ();
  void post();
  OCPI::Xfer::XferRequest::CompletionStatus getStatus();
  OCPI::Xfer::XferRequest &group(OCPI::Xfer::XferRequest* lhs);
  void modify(OCPI::Xfer::Offset new_offsets[], OCPI::Xfer::Offset old_offsets[]);
  XferRequest* copy(OCPI::Xfer::Offset srcoff,
		    OCPI::Xfer::Offset dstoff,
		    size_t nbytes,
		    XferRequest::Flags flags);
};

class XferServices : public OCPI::Xfer::XferServices, virtual public OCPI::Util::SelfMutex {
