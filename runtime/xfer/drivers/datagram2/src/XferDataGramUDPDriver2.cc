...
#include "OsMisc.hh"
#include "XferDataGramUDPDriver2.hh"

namespace OCPI {
namespace Xfer {
namespace UDP2 {

static const uint16_t DEFAULT_UDP_TRANSFER_PORT = 41000;
...
static uint16_t get_env_uint16(const char* name, uint16_t def);
static std::string get_env_string(const char* name, std::string def);
...
#define DRIVERNAME "datagram2-udp: "  // Name for debug messages
const char* protoname = "datagram_udp";
...
const char* DGXferFactory::getProtocol() {
    return "ocpi-udp-rdma";
}

// Register with the data transfer system
RegisterTransferDriver<DGXferFactory> udp2DatagramDriver;
    ...
    : EndPoint(a_factory, eps, other, a_local, a_size, params)
{
    ...
    if (local()) {
        // use the specified UDP local address
        std::string local_ip = get_env_string("OCPI_TRANSFER_IP_ADDRESS", "0.0.0.0");

        //TODO: check OCPI_SOCKET_INTERFACE if OCPI_TRANSFER_IP_ADDRESS is not specified

        // use the specified UDP local port number
        static uint16_t s_port = 0;
        if (s_port == 0) {
            s_port = get_env_uint16("OCPI_UDP_TRANSFER_PORT", DEFAULT_UDP_TRANSFER_PORT); }
        uint16_t local_port = s_port++;

        // create the socket
        m_socket = new UDPSocket(m_ifname, local_ip, local_port);
        ...
        if (parseRemoteEndpointInfo(protoInfo, remote_ifname, remote_addr, remote_udp_port)) {
            //TODO are there any more sanity checks we could perform here
            if (remote_ifname != "udp") {
            ...
            memset(&m_remote_sa, 0, sizeof(struct sockaddr_in));
            m_remote_sa.sin_family = AF_INET;
	        m_remote_sa.sin_port = htons(remote_udp_port);
            inet_aton(remote_addr.c_str(), &m_remote_sa.sin_addr);
        }
        else {
...
bool DGEndPoint::parseRemoteEndpointInfo(
    const std::string& protoInfo, std::string& ifname, std::string& ip_address, uint16_t& udp_port) {

    size_t protoSepPos = protoInfo.find(':');
    size_t addrSepPos = protoInfo.find(':', protoSepPos + 1);
    ...
    ifname = protoInfo.substr(0, protoSepPos);
    if (ifname != "udp") {
        return false;
    }

    // get the remote ip address
    ip_address = protoInfo.substr(protoSepPos + 1, addrSepPos - protoSepPos - 1);
    struct sockaddr_in sa;
    if (inet_pton(AF_INET, ip_address.c_str(), &(sa.sin_addr)) != 1) {
        return false;}

    // get the port number
    std::string port_s = protoInfo.substr(addrSepPos + 1);
    int port_i;
    if(sscanf(port_s.c_str(), "%d", &port_i) != 1) {
        return false;
    }

    // the protoInfo contains the DCP (control plane) port number
    // the FPGA uses the control plane port number + 1 for the 
    // data plane port number
    udp_port = static_cast<uint16_t>(port_i+1);

    return true;
}

struct sockaddr_in* DGEndPoint::Addr() {
    return &m_remote_sa;
}
...
////////////////////////////////////////////////////////////////////////////////
// UDPSocket
UDPSocket::UDPSocket(std::string& ifname, std::string& ip_addr, uint16_t portNum) :
    m_if(ifname), m_ipAddr(ip_addr), m_portNum(portNum) {
    ocpiInfo(DRIVERNAME "Creating udp socket on interface \"%s\"", ifname.c_str());
    ...
    if ((m_fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
        std::string error;
        OCPI::OS::setError(error, "opening udp socket");
    ...
    if (bind(m_fd, reinterpret_cast<sockaddr*>(&sa), sizeof(sa)) == -1) {
        ::close(m_fd);
        std::string error;
    ...
    ocpiInfo(DRIVERNAME "Opened UDP socket on interface \"%s\" (%d) with MTU %d", ifname.c_str(), m_if.index, m_if.mtu);
}

void UDPSocket::set_send_timeout(uint32_t timeout_ms) {
...
void UDPSocket::set_receive_timeout(uint32_t timeout_ms) {
...
void UDPSocket::send(struct iovec *iov, size_t iovlen, DGEndPoint& dest_ep) {
    
    // Prepare message header structure
    struct msghdr hdr;
    memset(&hdr, 0, sizeof(struct msghdr));
    hdr.msg_name = dest_ep.Addr();
...
size_t UDPSocket::receive(uint8_t *buffer, size_t buflen) {
...
size_t UDPSocket::mtu() const {
    return (size_t) m_if.mtu - 32; // for IP and UDP headers
}
...
void RxThread::run() {
    ...
    while(m_running) {
        ...
        FrameHeader hdr = FrameHeader::from_bytes(rx_buf + 2);
        DGXferServices *xs = m_ep.getXfer(hdr.dst_id);
        if (xs) {
            xs->processFrame(hdr, rx_buf + 2 + FrameHeader::LENGTH, length - FrameHeader::LENGTH - 2);
        }
        else {
    ...
    while (txn_bytes_left > 0) {
        ...
        if (current_frame_space > MsgHeader::LENGTH) {
            ...
            if (data_len != 0) {
                ...
                hdr.msg_seq += static_cast<uint16_t>(1);
            }
        }
...
void OutboundFrame::to_iovs(std::vector<struct iovec>& iovs) {
    ...
    // padding at start of udp payload
    iov.iov_base = &pad;
    iov.iov_len = 2;
    iovs.push_back(iov);
