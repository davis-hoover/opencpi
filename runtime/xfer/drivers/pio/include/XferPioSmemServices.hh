...
namespace PIO {
  ...
  public:
    ...
    static void mapEndPoint(EndPoint &ep, std::string &out) {
      const char *semi = strrchr(ep.name().c_str(), ';');
      assert(semi);
      OU::format(out, "/%.*s", (int)(semi - ep.name().c_str()), ep.name().c_str());
  ...
  class BaseSmemServices : public OCPI::Xfer::SmemServices
  {
  ...
  public:
    ...
    BaseSmemServices (EndPoint &loc )
      : OCPI::Xfer::SmemServices(loc ), m_pSmem (0)
      {
      }
