...
#define EPNAME "ocpi-scif-dma"
	...
	ocpiDebug("SCIF DMA ep %p %s: size = 0x%zx port = %d ",
		  this, protoInfo, size, m_portID.port);
	...
	return *new EndPoint(*this, protoInfo, eps, other, local, m_node, m_port);
      }
    };
    ...
    getStatus () {
      ...
      //      printf("&&&&&&&&& ******* Status =%d\n", *((int*)parent().m_sourceSmb->map((Offset)m_addr,4)) );
      if (  *((int*)parent().m_sourceSmb->map((Offset)m_addr,4)) ) {
...
#define TRACE_XFERS  
      ...
      if ( last ) {
	//	printf("***** LAST TX !!\n");	
	*((int*)parent().m_sourceSmb->map((Offset)m_addr,4)) = 0;
	uint64_t v =  *((int*)parent().m_sourceSmb->map((Offset)transfer->src_off,4)) |
	  *((int*)parent().m_sourceSmb->map((Offset)transfer->src_off,4)) << 31;
