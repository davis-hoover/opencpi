...
class TestBase {
public:
  TestBase( XF::XferFactory * factory, std::string & ep )
  ...
  void produce( int sbid, int tbid, uint32_t nbytes ) {
    ...
    //    ocpiAssert(     m_reqs [ sbid ] [ tbid ] ->getStatus() == XF::XferRequest::CompleteSuccess );
...
#ifdef DEBUG
    while (  m_reqs [ sbid ] [ tbid ] ->getStatus() != XF::XferRequest::CompleteSuccess ) {      
...
protected:
  XF::XferFactory  * m_factory;
  std::string                  m_endpoint_url;
  XF::EndPoint     * m_endpoint;
  XF::SmemServices * m_smem;
  MemLayout                  * m_Txmem;
  MemLayout                  * m_Rxmem;
  XF::XferServices * m_xferServices;
  std::string                  m_other_endpoint_url;
  XF::EndPoint     * m_other_endpoint;
  XF::SmemServices * m_other_smem;

  // transfer lists
  XF::XferRequest * m_reqs[ BUFFER_COUNT] [ BUFFER_COUNT ];
...
public:
  ServerTest( XF::XferFactory * factory, std::string & ep )
    :TestBase(factory,ep){}
...
public:
  ...
  ClientTest( XF::XferFactory * factory, std::string & ep  )
    :TestBase(factory,ep){}
  ...
  void connect( std::string & ) {
    ...
    server_init_req->copy( s_start_off + url_off, url_off, 256, XF::XferRequest::DataTransfer );
    server_init_req->copy( s_start_off + con_off, con_off, 4, XF::XferRequest::FlagTransfer );    
    ...
    while( server_init_req->getStatus() != XF::XferRequest::CompleteSuccess ) {
...
int main( int argc, char** argv )
{
  ...
  XF::XferFactory * factory = fm.find(config.endpoint.c_str());
  ocpiAssert( factory );
