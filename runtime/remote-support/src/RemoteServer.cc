...
// This is the pretty generic server object that is discoverable by clients, and also receives
// connection requests from clients.  The there is a virtual "addClient" method that
// inheritors of this class use to create more specific session objects for a client.
// It might be useful outside this (current) remote container context.
// I.e. this file does not know about being a "container server".
  ...
  namespace Remote {
    ...
    bool Server::doit(std::string &error) {
      ...
      *fds = m_alwaysSet;
      ...
      timeout[0].tv_sec = (decltype(timeout[0].tv_sec))(m_sleepUsecs / 1000000);
      timeout[0].tv_usec = (decltype(timeout[0].tv_usec))(m_sleepUsecs % 1000000);
      errno = 0;
    ...
    receiveDisc(std::string &error) {
      ...
      if (m_disc->receive(rFrame, length, 0, from, error, &index)) {
        ...
        if (index) {
          ...
          cp += m_name.length();
          *cp++ = '\n';
          if (m_discoveryInfo.length() + 1 > (size_t)(OE::MaxPacketSize - OCPI_SIZE_T_DIFF(cp, start))) {
