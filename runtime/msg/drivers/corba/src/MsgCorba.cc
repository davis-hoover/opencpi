...
namespace CORBA {
      ...
      class XferFactory;
      class XferServices;
      class MsgChannel : public TransferBase<XferServices,MsgChannel>
      {
      private:
	...
	MsgChannel(XferServices & xf,
		   ...
		   const char  * url,
		   const OB::PValue *ourParams,
		   const OB::PValue *otherParams)
	  : TransferBase<XferServices,MsgChannel>(xf, *this),
	    m_url(url), m_circuit(0)
	{
	  ...
	  for (RdmaEndpoints::const_iterator i = m_rdmaEndpoints.begin();
	    ...
	    try {
	      m_circuit = &MessageEndpoint::connect(i->c_str(), 4096, info.c_str(), NULL);
	    } catch (...) {
#ifndef NDEBUG
	...
	OT::BufferUserFacet*  getNextEmptyOutputBuffer(uint8_t *&data, size_t &length)
	{
	  return m_circuit->getNextEmptyOutputBuffer(data, length, NULL);
	}

	void sendOutputBuffer(OT::BufferUserFacet* b, size_t msg_size, uint8_t opcode )
	{
	  m_circuit->sendOutputBuffer(b, msg_size, opcode);
	}

	OT::BufferUserFacet*  getNextFullInputBuffer(uint8_t *&data, size_t &length,
								      uint8_t &opcode)
	{
	  return m_circuit->getNextFullInputBuffer(data, length, opcode);
	}

	void releaseInputBuffer(OT::BufferUserFacet* b)
	{
      ...
      class XferServices : public ConnectionBase<XferFactory,XferServices,MsgChannel>
      {
	const OM::Protocol &m_protocol;
      public:
	XferServices ( const OM::Protocol & protocol , const char  * other_url, 
		       const OB::PValue *our_props=0,
		       const OB::PValue *other_props=0 );
	...
	virtual ~XferServices ()
	{
      ...
      class Device
	: public DeviceBase<XferFactory,Device>
      {
      public:
	Device(const char* a_name)
	  : DeviceBase<XferFactory,Device>(a_name, *this)
	{
	...
	virtual ~Device(){}
      ...
      class XferFactory
	: public DriverBase<XferFactory, Device, XferServices,
					       msg_transfer>
      {
	...
	XferServices* m_services;
      ...
      XferServices ( const OM::Protocol &a_protocol , const char  * other_url,
		     const OB::PValue *ourParams,
		     const OB::PValue *otherParams)
	: ConnectionBase<XferFactory,XferServices,MsgChannel>
	...
	// Empty
      ...
      Device::
      configure(ezxml_t x) {
	OCPI::Base::Plugin::Device::configure(x);
      }
