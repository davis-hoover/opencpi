...
PortMetaData( PortOrdinal pid,
	      ...
	      XF::EndPoint *ep,
              ...
              PortSet* psmd )
  : OU::Child<PortSet,PortMetaData>(*psmd, *this), m_shadow(true), remoteCircuitId(0),
    ...
    m_bufferData(NULL)
{
  ...
  output =  (m_descriptor.type == ProducerDescT) ? true : false;
  real_location_string = ep.name();
  init( );
}
...
PortMetaData( PortOrdinal pid, 
  ...
  : OU::Child<PortSet,PortMetaData>(*psmd, *this),m_shadow(true),remoteCircuitId(0),
    ...
    m_bufferData(NULL)
{
  m_descriptor.type = s ? ProducerDescT : ConsumerDescT;
  m_descriptor.role = s ? ActiveMessage : ActiveFlowControl;
  if ( ep ) {
    real_location_string = ep->name();
  }
  if ( shadow_ep ){
...
PortMetaData( PortOrdinal pid, 
              XF::EndPoint &ep, 
              XF::EndPoint &shadow_ep, 
              const Descriptors &pdd, 
              PortSet* psmd )
  : OU::Child<PortSet,PortMetaData>(*psmd, *this),m_shadow(true),
