 ...
 * Constructors
 *********************************/
InputBuffer::InputBuffer(Port* port, OCPI::OS::uint32_t tid)
...
{
  m_feedbackDesc.type = ProducerDescT;
  m_bVaddr = 0;
  m_bsVaddr = 0;
  m_bmdVaddr = 0;
...
void InputBuffer::update(bool critical)
{
  ...
  for ( n=0; n<m_outputPortCount; n++ ) {
    ...
    if ( !m_rssVaddr[idx] && input_offsets->myShadowsRemoteStateOffsets[idx] ) {
      ...
      this->m_feedbackDesc.type = ConsumerFlowControlDescT;
...
{
  return reinterpret_cast<Port*>(getPort())->isShadow();
}
...
#ifdef LEAST_BUSY
  Port* sp = static_cast<Port*>(this->getPort());
  if ( state->pad != 0) {
...
volatile BufferMetaData* InputBuffer::getMetaData()
{
  ...
  PortSet* s_port = static_cast<PortSet*>(getPort()->getCircuit()->getOutputPortSet());
  ocpiAssert(!isShadow());
...
{
  PortSet* s_port = static_cast<PortSet*>(getPort()->getCircuit()->getOutputPortSet());
  PortOrdinal id = s_port->getPortFromIndex(idx)->getPortId();
  return &m_sbMd[0][id];
}
