...
Port::Port(PortMetaData* data, PortSet* ps)
  : OU::Child<PortSet,Port>(*ps, *this), OCPI::Time::Emit( ps, "Port", "",NULL ), 
 ...
 *********************************/
const Descriptors * Port::
finalize(const Descriptors *other, Descriptors &mine,
	 Descriptors *flow, bool &done)
{
  const Descriptors *result = &mine;
  Circuit &c = *getCircuit();
  Port *otherPort;
  done = true;
  if (m_data->output) {
    ocpiAssert(mine.type == ProducerDescT);
    switch (mine.role) {
    case ActiveFlowControl:
	...
	*flow = c.getInputPortSet(0)->getPort(0)->getMetaData()->m_shadowPortDescriptor;
	flow->type = ConsumerFlowControlDescT;
	result = flow;
      }
      break;
    }
...
void 
Port::
setFlowControlDescriptorInternal( const Descriptors & desc )
{
 ...
 *********************************/
void Port::
getPortDescriptor(Descriptors& desc, const Descriptors *other) {
  ...
  if ( ! isOutput() ) { 
    ocpiAssert(desc.type == ConsumerDescT);
    if (desc.role == NoRole)
      desc.role = ActiveFlowControl;
    ...
    ocpiAssert(desc.type == ProducerDescT);
    if (desc.role == NoRole)
      desc.role = ActiveMessage;
...
~Port()
{
  ...
  for (XF::TemplateMapIter tmi = m_templates.begin(); tmi != m_templates.end(); tmi++)
    tmi->second->release();
}
...
static void
addOffset(OU::VList& offsets, XF::Offset from_base, XF::Offset to_base,
	  size_t adjust, const char *debug) {
  (void)debug;
  Port::ToFrom* tf = new Port::ToFrom;
  XF::Offset adj = OCPI_UTRUNCATE(XF::Offset, adjust);
...
void Port::
getOffsets( XF::Offset to_base_offset, OU::VList& offsets )
{
  XF::Offset
    from_offset = m_offsetsOffset,
    to_offset = to_base_offset,
    increment = OCPI_UTRUNCATE(XF::Offset, sizeof(PortMetaData::BufferOffsets));
...
createInputOffsets() 
{
  OCPI::Util::ResAddrType boffset, moffset, soffset;
    ...
    if ( local ) {
      ...
      m_data->m_shadowPortDescriptor.role = ActiveMessage;
      m_data->m_shadowPortDescriptor.type = ConsumerFlowControlDescT;
