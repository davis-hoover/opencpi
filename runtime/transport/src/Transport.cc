...
Transport( TransportManager *tpg, bool uses_mailboxes, OCPI::Time::Emit * parent  )
  : OCPI::Time::Emit(parent, "Transport"),
...
init() {
  if ( g_nextCircuitId == 0 ) {
    ...
    g_nextCircuitId = (unsigned)getpid() + (unsigned)rand();
  }
...
getMailBoxLock( const char* mbid )
{
  ...
  unsigned len = m_mailbox_locks.getElementCount();
...
void
Transport::
fillDescriptorFromEndPoint(XF::EndPoint &ep, Descriptors &desc) {
...
Port * 
Transport::
createOutputPort(Descriptors& outputDesc, const Descriptors& inputDesc) {
...
Port *Transport::
createOutputPort(Descriptors& outputDesc, Port &inputPort) {
...
Port *Transport::
createInputPort(Descriptors& desc, const OB::PValue */*params*/)
{
...
sendOffsets( OU::VList& offsets, XF::EndPoint &remote_ep, 
	     size_t extraSize, XF::Offset extraFrom, XF::Offset extraTo)
{
  OU::AutoMutex guard ( m_mutex, true ); 
