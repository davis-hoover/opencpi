...
public:
  PortSet(Circuit &c, bool isOutput, DataDistribution &dd, unsigned bufferCount,
	  ...
	  OCPI::Xfer::EndPoint *outputEp = NULL,  // non-NULL when we are creating ports
	  OCPI::Xfer::EndPoint **inputEp = NULL,  // non-NULL when are are creating ports, port port
	  const Descriptors *inputDesc = NULL);
  ~PortSet();
