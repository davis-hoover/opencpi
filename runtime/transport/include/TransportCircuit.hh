...
namespace OCPI {
  ...
  namespace Transport {
    ...
    public:
       ...
       *********************************/
      bool updateConnection(Port*,uint32_t count);

      /**********************************
       * Adds a port to the circuit
       *********************************/
      void addInputPort(OCPI::Xfer::EndPoint &iep,
			const Descriptors& inputDesc,
			OCPI::Xfer::EndPoint &oep);

      /**********************************
       * Adds a port to the circuit
       *********************************/
      Port* addPort( PortMetaData* pmd );        
       ...
       *********************************/
      void setFlowControlDescriptor( Port* p, 
       ...
       *********************************/
      //      bool updateInputs();
      void updateInputs(ContainerComms::RequestUpdateCircuit*);
      ...
      PullDataDriver* createPullDriver( const Descriptors& pdesc);
    ...
    protected:
       ...
       *********************************/
      //      void
      //        getUserPortFlowControlDescriptor( Descriptors* fb, unsigned int idx );
      ...
      OCPI::Util::VList m_queuedTransfers[MAX_PCONTRIBS];

      // ZCopy transfer list.  This list is used to Q up zero copy transfers of another
      // ports buffers
      OCPI::Util::VList m_queuedInputOutputTransfers[MAX_PCONTRIBS];
    ...
    protected:
      ...
      // On client, it is passed into the OCPI::Transport::createCircuit, which passes it into
