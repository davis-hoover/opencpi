...
protected:
  ...
  //  virtual void bufferFull(Port *port);

  // indicate that a remote input buffer has been freed.
  // Since we manage circular buffers, the actual buffer is implied (next)
  virtual void freeBuffer(Port *port);
...
public:
  ...
  virtual bool hasFullInputBuffer(Port *, InputBuffer **) const;
  // determine if there is an available buffer, but does not affect the
  virtual bool hasEmptyOutputBuffer(Port *port) const;
  // get the next available buffer from the specified output port
  virtual Buffer* getNextEmptyOutputBuffer(Port *src_port);
  // get the next available buffer from the specified input port
  virtual Buffer* getNextFullInputBuffer(Port *input_port);
  // indicate that all remote input buffer has been emptied.
  virtual void freeAllBuffersLocal(Port *port);
  // mark the input buffer as "Empty" and informs interested outputs that the input is now available.
  virtual void consumeAllBuffersLocal(Port *port);
...
class Controller2 : public Controller {
  Port *m_inputPort; // Port to produce to
public:
  Controller2(PortSet &output, PortSet &input);
  virtual ~Controller2(){};
  void createOutputTransfers(Port &s_port);
  virtual void addTransferPreState(OCPI::Xfer::XferRequest *pt, Port &s_port,
...
public:
  ...
  virtual ~Controller3(){};
  void addTransferPreState(OCPI::Xfer::XferRequest* pt, Port &s_port, unsigned s_tid,
