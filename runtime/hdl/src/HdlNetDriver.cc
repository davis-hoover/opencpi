...
namespace OCPI {
  namespace HDL {
	...
	eh_out.length = htons((uint16_t)(sizeof(eh_out)-2u + bytes));
	if (cmd && bytes)
	...
	nop.header.etherTypeOverlay = 0; // for valgrind
        nop.header.tag = 0;
	nop.header.length = htons((uint16_t)(sizeof(nop)-2u));
	nop.header.pad = 0;
	...
	if (response.header.length == htons((uint16_t)(sizeof(response)-2u)) &&
