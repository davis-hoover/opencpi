...
namespace OCPI {
  namespace HDL {
    ...
    init(std::string &err) {
      ...
      try {
        if (sigsetjmp(jmpbuf, 1) == 0) {
          ...
          // The uint type we are using to iterate over the bytes in the magic number.
          const unsigned iter_size = 8;
          const size_t num_iter = (sizeof(magic) * 8u)/iter_size;
          for (unsigned i = 0; i < num_iter; i++) {
            ...
            const uint8_t magic_slice =	(uint8_t)(OCCP_MAGIC >> (i*8));
	    // was OCCP_MAGIC << (((num_iter - 1) - i) * iter_size) >> (64 - (64 / num_iter));
            if (magic_slice != curr) {
    ...
    print() {
      ...
      time_t bsbd = (time_t)m_UUID.birthday;
    ...
    initAdmin(OccpAdminRegisters &admin, const char *a_platform, HdlUUID &hdlUuid,
      ...
      memset((void*)&admin, 0, sizeof(admin));
