...
#define OCPI_DATA_TYPE_S OCPI_DATA_TYPE
    ...
    private:
      ...
      // These two methods hide the fact that we need two different members for error strings
      // in two different structures for C and C++
      const char *errorString() const {
	return
	  m_context->errorString ? m_context->errorString : // if the C error string is set, use it˜
	  m_errorString.empty() ? NULL : m_errorString.c_str();
      }
	...
	m_errorString.clear();
      ...
      mutable std::string m_errorString;         // error string set via "setError"
    protected:
