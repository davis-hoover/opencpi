...
int test_AD9361FSKConfigurator() {
  // RX CHANNEL
  int ret = 0;
  AD9361FSKConfigurator uut;
  try {
    std::vector<const char*> data_stream_rx;
    data_stream_rx.push_back("rx1");
    data_stream_rx.push_back("rx2");
    for(auto it=data_stream_rx.begin (); it!=data_stream_rx.end(); ++it) {
      ...
      TEST(*it, di, (int32_t)data_stream_direction_t::tx, false, 0., false);
      uut.unlock_all();
      // Tuning Freq (MHz) [39.28 MHz - 6030.72 MHz]
      // ================================================
      // LOWER BOUNDS
      ...
      uut.unlock_all();
      // UPPER BOUNDS
      TEST(*it, fc, 6030.72 , true, 0.000001, true )
      uut.unlock_all();
      TEST(*it, fc, 6030.71 , true, 0.000001, false)
      uut.unlock_all();
      // Bandwidth (MHz) [0.2/8192 MHz - 14.0 MHz]
      // ================================================
      //  LOWER BOUNDS
      TEST(*it, bw, 0.2/8192.-0.01, true, 0.000001, false)
      uut.unlock_all();
      TEST(*it, bw, 0.2/8192.,true, 0.000001, true )
      uut.unlock_all();
      // MEDIAN BOUND
      ...
      uut.unlock_all();
      // Sampling rate (Msps) [2.083334/8192 - 15.36]
      // @TODO Check Values
      // ================================================
      // LOWER BOUNDS
      TEST(*it, fs, 2.083334/8192.-0.01    , true, 0.000001, false)
      uut.unlock_all();
      TEST(*it, fs, 2.083334/8192., true, 0.000001, true )
      uut.unlock_all();
      // MEDIAN BOUND
