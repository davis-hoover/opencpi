...
/// @todo / FIXME - Implement strong guarantee of exception safety.
void Configurator::unlock_config(
    const data_stream_ID_t data_stream_key,
    const config_key_t     config_key) {
...
void Configurator::unlock_config(
  ...
  config.unlock();
  log_debug("configurator: unlocked config: %s", config_key.c_str());
...
/// @todo / FIXME - Implement strong guarantee of exception safety.
config_value_t Configurator::get_config_min_valid_value(
    const data_stream_ID_t data_stream_key,
    const config_key_t     config_key) {
...
/// @todo / FIXME - Implement strong guarantee of exception safety.
config_value_t Configurator::get_config_max_valid_value(
    const data_stream_ID_t data_stream_key,
    const config_key_t     config_key) {
...
/// @todo / FIXME - Implement strong guarantee of exception safety.
bool Configurator::get_config_is_locked(
    const data_stream_ID_t data_stream_key,
    const config_key_t     config_key) {
...
void Configurator::log_all_possible_config_values(
    bool do_info, bool do_debug) {

  bool any = false;
  any |= do_info  and log_info();
  any |= do_debug and log_debug();
  if(not any) {
    return;
  }
...
const ConfigValueRanges& Configurator::get_ranges_possible(
    const data_stream_ID_t data_stream_key,
    const config_key_t     config_key) {
  // required because this method's behavior could be affected by
  // the impose_constraints() call we couldn't put in the
...
void Configurator::ensure_impose_constraints_first_run_did_occur() {
...
void Configurator::throw_if_any_possible_ranges_are_empty(
...
void Configurator::impose_constraints() {
  ...
  for(int iter_num=0; en & (iter_num < max); iter_num++) {
    ...
    for(; itds!= m_data_streams.end(); itds++) {
      ...
      data_stream_t stream_orig = data_streams_orig.at(itds->first);
...
LockRConstrConfig& Configurator::get_config(
    const data_stream_ID_t data_stream_key,
...
const LockRConstrConfig& Configurator::get_config(
    const data_stream_ID_t data_stream_key,
...
LockRConstrConfig& Configurator::get_config(const config_key_t cfg_key){
  try {
...
bool Configurator::constrain_all_XY_such_that_X_equals_Y(
...
bool Configurator::constrain_X_to_function_of_A_and_B(
...
bool Configurator::constrain_X_to_A_divided_by_B(
    LockRConstrConfig& cfg_X,
...
bool Configurator::constrain_all_XAB_such_that_X_equals_A_multiplied_by_B(
    LockRConstrConfig& cfg_X,
...
bool Configurator::constrain_all_XAB_such_that_X_equals_A_divided_by_B(
    LockRConstrConfig& cfg_X,
    LockRConstrConfig& cfg_A, LockRConstrConfig& cfg_B) const {
...
bool Configurator::constrain_all_XAB_such_that_X_equals_A_plus_B(
    LockRConstrConfig& cfg_X,
    LockRConstrConfig& cfg_A, LockRConstrConfig& cfg_B) const {
...
bool Configurator::constrain_X_to_A_multiplied_by_B(
    LockRConstrConfig& cfg_X,
    const LockRConstrConfig& cfg_A, const LockRConstrConfig& cfg_B) const {
...
bool Configurator::constrain_Y_less_than_or_equal_to_X(
...
bool Configurator::constrain_Y_equals_constant(
...
bool Configurator::limit_Y_to_less_than_or_equal_to_X(
...
bool Configurator::limit_Y_to_greater_than_or_equal_to_X(
    LockRConstrConfig& cfg_Y,
    const LockRConstrConfig& cfg_X) const {
