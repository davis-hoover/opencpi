...
AnaRadioCtrlr::AnaRadioCtrlr(const char* descriptor,
    Configurator& configurator) :
    m_descriptor(descriptor), m_configurator(configurator) {
}

#if 0
const std::string& AnaRadioCtrlr::get_descriptor() const {
  return m_descriptor;
}
#endif

const std::vector<ConfigLock>& AnaRadioCtrlr::get_config_locks() const {
  return m_config_locks;
}

ConfigValueRanges
AnaRadioCtrlr::get_ranges_possible_tuning_freq_MHz(
...
ConfigValueRanges
AnaRadioCtrlr::get_ranges_possible_bandwidth_3dB_MHz(
    const data_stream_ID_t data_stream_ID) const {
...
AnaRadioCtrlr::get_ranges_possible_gain_mode(
    const data_stream_ID_t data_stream_ID) const {
...
ConfigValueRanges
AnaRadioCtrlr::get_ranges_possible_gain_dB(
    const data_stream_ID_t data_stream_ID) const {
...
/*void AnaRadioCtrlr::log_all_possible_config_values() {
  m_configurator.log_all_possible_config_values();
}*/

bool AnaRadioCtrlr::request_config_lock(
...
void AnaRadioCtrlr::unlock_config_lock(
...
bool AnaRadioCtrlr::lock_tuning_freq_MHz(
  ...
  if(did_lock) {
    ...
    log_info_config_lock_tol(did_lock, is, ds_ID, val, tol, cfg_key, "MHz", &meas);
  }
  else {
    log_info_config_lock_tol(did_lock, is, ds_ID, val, tol, cfg_key, "MHz");
  }

  return did_lock and is;
}

bool AnaRadioCtrlr::lock_bandwidth_3dB_MHz(
    const data_stream_ID_t ds_ID,
    const config_value_t   val,
    const config_value_t   tol) {
...
void AnaRadioCtrlr::unlock_tuning_freq_MHz(
    const data_stream_ID_t data_stream_ID) {
  
  unlock_config(data_stream_ID, config_key_tuning_freq_MHz);
}

void AnaRadioCtrlr::unlock_bandwidth_3dB_MHz(
    const data_stream_ID_t data_stream_ID) {

  unlock_config(data_stream_ID, config_key_bandwidth_3dB_MHz);
}

void AnaRadioCtrlr::unlock_config(
    const data_stream_ID_t ds_ID,
...
bool AnaRadioCtrlr::do_min_data_stream_config_locks(
    const data_stream_ID_t data_stream_ID,
...
void AnaRadioCtrlr::throw_if_data_stream_lock_request_malformed(
...
bool AnaRadioCtrlr::config_val_is_within_tolerance(
...
void AnaRadioCtrlr::throw_if_data_stream_disabled_for_write(
    const data_stream_ID_t& ds_ID,
...
void AnaRadioCtrlr::throw_if_data_stream_disabled_for_read(
    const data_stream_ID_t& ds_ID,
    const char* config_description) const {
...
void AnaRadioCtrlr::log_info_config_lock_tol(const bool did_lock, const bool is,
    const data_stream_ID_t& ds_ID,
...
DigRadioCtrlr::DigRadioCtrlr(const char *descriptor, Configurator &configurator)
  : AnaRadioCtrlr(descriptor, configurator) {
}

ConfigValueRanges
DigRadioCtrlr::get_ranges_possible_sampling_rate_Msps(
    const data_stream_ID_t data_stream_ID) const {
...
std::vector<bool>
DigRadioCtrlr::get_ranges_possible_samples_are_complex(
    const data_stream_ID_t data_stream_ID) const {
...
bool DigRadioCtrlr::request_config_lock(
    const config_lock_ID_t   config_lock_ID,
    const ConfigLockRequest& config_lock_request) {
...
void DigRadioCtrlr::unlock_config_lock(
    const config_lock_ID_t config_lock_ID) {
...
bool DigRadioCtrlr::lock_sampling_rate_Msps(
    const data_stream_ID_t ds_ID,
    const config_value_t   val,
    const config_value_t   tol) {
  ...
  if(did_lock) {
    ...
    log_info_config_lock_tol(did_lock, is, ds_ID, val, tol, cfg_key, "Msps", &meas);
  }
  else {
    log_info_config_lock_tol(did_lock, is, ds_ID, val, tol, cfg_key, "Msps");
  }

  return did_lock and is;
}

bool DigRadioCtrlr::lock_samples_are_complex(
    const data_stream_ID_t  ds_ID,
    const bool              val) {
...
bool DigRadioCtrlr::lock_gain_mode(
    const data_stream_ID_t  ds_ID,
...
bool DigRadioCtrlr::lock_gain_dB(
    const data_stream_ID_t ds_ID,
    const config_value_t   val,
    const config_value_t   tol) {
  ...
  if(did_lock) {
    ...
    log_info_config_lock_tol(did_lock, is, ds_ID, val,tol,cfg_key,"dB",&meas);
  }
  else {
    log_info_config_lock_tol(did_lock, is, ds_ID, val, tol, cfg_key, "dB");
  }

  return did_lock and is;
}

void DigRadioCtrlr::unlock_sampling_rate_Msps(
    const data_stream_ID_t data_stream_ID) {

  this->unlock_config(data_stream_ID, config_key_sampling_rate_Msps);
}

void DigRadioCtrlr::unlock_samples_are_complex(
    const data_stream_ID_t data_stream_ID) {

  this->unlock_config(data_stream_ID, config_key_samples_are_complex);
}

void DigRadioCtrlr::unlock_gain_mode(
    const data_stream_ID_t data_stream_ID) {

  this->unlock_config(data_stream_ID, config_key_gain_mode);
}

void DigRadioCtrlr::unlock_gain_dB(
    const data_stream_ID_t data_stream_ID) {

  this->unlock_config(data_stream_ID, config_key_gain_dB);
}

bool DigRadioCtrlr::do_min_data_stream_config_locks(
    const data_stream_ID_t             data_stream_ID,
    const DataStreamConfigLockRequest& data_stream_config_lock_request) {
  ...
  // first perform all the analog-specific config locks
  {
    ...
    if(not AnaRadioCtrlr::do_min_data_stream_config_locks(ds_ID, req)) {
      return false;
    }
  }
...
void DigRadioCtrlr::log_info_config_lock(const bool did_lock,
...
void DigRadioCtrlr::log_info_ds_cfg_lock_req_vals(
...
void DigRadioCtrlr::throw_if_data_stream_lock_request_malformed(
    const DataStreamConfigLockRequest& data_stream_config_lock_request) const {

  const DataStreamConfigLockRequest& req = data_stream_config_lock_request;

  // first check all the analog-specific config locks
  AnaRadioCtrlr::throw_if_data_stream_lock_request_malformed(req);
