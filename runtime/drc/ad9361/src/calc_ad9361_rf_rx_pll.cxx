...
const char* calc_AD9361_Rx_RFPLL_ref_divider(
  ...
  uint8_t Rx_Ref_Divider_1_0 = (uint8_t)(Rx_Ref_Divider_1 << 1) | Rx_Ref_Divider_0;
...
const char* calc_AD9361_Rx_RFPLL_LO_freq_Hz(
  ...
  { // restrict scope so we don't accidentally use non-double values
    ...
    {
      float& f = Rx_RFPLL_ref_divider;
      const char* ret = calc_AD9361_Rx_RFPLL_ref_divider(f, regs);
      if(ret != 0) {
        return ret;
      }
    }
    ...
    {
      uint16_t& u16 = Rx_RFPLL_N_Integer;
      const char* ret = calc_AD9361_Rx_RFPLL_N_Integer(u16, regs);
      if(ret != 0) {
        return ret;
      }
    }
    ...
    {
      uint32_t& u32 = Rx_RFPLL_N_Fractional;
      const char* ret = calc_AD9361_Rx_RFPLL_N_Fractional(u32, regs);
      if(ret != 0) {
        return ret;
      }
    }
    ...
    {
      uint8_t& u8 = Rx_RFPLL_VCO_Divider;
      const char* ret = calc_AD9361_Rx_RFPLL_VCO_Divider(u8, regs);
      if(ret != 0) {
        return ret;
      }
    }
