...
#include "OCL_Worker.hh"
#include "OclContainer.hh"
#include "OclDevice.hh"
#include "OclPlatformManager.hh"
...
// But it does not complain at compile time.
	...
	m_oclWorker->logLevel = OCPI_UTRUNCATE(uint8_t, OS::Log::getLevel());
...
#define PUT_GET_PROPERTY(n)						         \
	...
	for (unsigned n = 0; n < m_nPorts; n++, oclp++) {
	  ...
	  oclp->isConnected = (connectedPorts() & (1u << n)) != 0;
