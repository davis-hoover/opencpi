...
PortSet(Circuit &c, bool isOutput, DataDistribution &dd, unsigned bufferCount,
	...
	DataTransfer::EndPoint *outputEp,        // non-NULL when we are creating ports
	DataTransfer::EndPoint **inputEp,        // non-NULL when are are creating ports, port port
	const OCPI::RDT::Descriptors *inputDesc) // when we are creating ports, per port
