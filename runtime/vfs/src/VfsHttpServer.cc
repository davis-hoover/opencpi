...
OCPI::VFS::Http::Server::
g_myDefaultLogger;

OCPI::Logger::ProducerName
OCPI::VFS::Http::Server::
g_myProducerName ("OCPI::VFS::Http::Server");

OCPI::VFS::Http::Server::
Server (OCPI::VFS::Vfs * fs,
...

{
  ...
  OCPI::Util::Http::Headers::const_iterator hit;
...

{
  ...
  OCPI::Util::Http::Headers::const_iterator ims = m_requestHeaders.find ("If-Modified-Since");
  ...
  if (m_requestType != REQUEST_HEAD) {
    ...
    while (!fileStream->eof() && m_outConn->good()) {
      ...
      totalBytes += (decltype(totalBytes))amountRead;
...

{
  ...
  if (m_majorVersion == 1 && m_minorVersion == 1) {
    OCPI::Util::Http::Headers::const_iterator it = m_requestHeaders.find ("Transfer-Encoding");
  ...
  {
    OCPI::Util::Http::Headers::const_iterator it = m_requestHeaders.find ("Content-Length");
  ...
  while (m_inConn->good() && fileStream->good()) {
    ...
    while ((!chunked || chunkSize > 0) &&
      ...
      if (chunked) {
        chunkSize -= (decltype(chunkSize))amountRead;
      }
      else if (haveContentLength) {
        contentLength -= (decltype(chunkSize))amountRead;
      }

      totalBytes += (decltype(chunkSize))amountRead;
...

{
  ...
  m_outConn->write (statusLine.data(), (std::streamsize)statusLine.length());
