...
OCPI::VFS::LoadableModule::
LoadableModule (OCPI::VFS::Vfs * vfs,
...

{
  ...
  if (g_loadableModulePrefix.length()) {
    ...
    relFileName += "_";
    relFileName += OCPI::VFS::relativeName (fileName);
  }
  else {
    relFileName = OCPI::VFS::relativeName (fileName);
  }

  std::string absFileName = OCPI::VFS::joinNames (g_loadableModuleLocation,
                                                       relFileName);
