...
define OcpiParseXml
    $(if $(call DoShell,$(ToolsDir)/ocpigen -R $1/$2.xml 2>/dev/null|tr "\n" ";"|tr " " "&",OcpiProps),\
    $(error ocpigen -R failed),$(foreach var,$(subst ;, ,$(OcpiProps)),$$(eval $(subst &, ,$(var)))))
    $(foreach var,$(subst ;, ,$(OcpiProps)),$(eval $(subst &, ,$(var))))
    $$(infox $0: OcpiProps is $$(OcpiProps) and $(OcpiProps))
endef
...
define OcpiSetProjectX
  ...
  ifneq ($(wildcard $1/Project.xml),)
    ...
    $$(infox Could not find Project.xml, using Project.mk)
...
define OcpiSetAsset
  ...
  # Library
  ifeq ($2,Library)
    ifneq ($$(wildcard $1/$(CwdName).xml),)($$(wildcard $1/$2.mk),)
      $$(warning Both Library.xml and $(CwdName.xml) were found, using $(CwdName).xml)
    endif
    ifneq ($$(wildcard $1/$(CwdName).xml),)
      $(call OcpiParseXml,$1,$(CwdName) in $(realpath .))
    else
