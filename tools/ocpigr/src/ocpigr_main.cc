...
//////////
void OcpigrObj::
addProperty(YAML::Emitter& emitter, OM::Worker &w, OM::Property *p) {
...
//////////
const char* OcpigrObj::
incompatibleType(OM::Worker& w, const char* tag, const char* val) {
...
genOcpiBlockTree(void) {
  ...
  for (StringSetIter it = specs.begin(); it != specs.end(); it++) {
    ...
    size_t equalVectorLength = compareBlockVectors(currBlockVector, prevBlockVector);
    ocpiDebug("equalVectorLengh: %zd", equalVectorLength);
    
    // Close finished sequences and maps
    size_t treeDepthChange = equalVectorLength - currTreeDepth;
    endTreeSeq(emitter, treeDepthChange);
    currTreeDepth = equalVectorLength;
    ocpiDebug("treeDepthChange: %zd", treeDepthChange);
    ocpiDebug("endTreeSeq currTreeDepth: %zd", currTreeDepth);

    // Create needed sequences of maps for tree
    createTreeSeq(emitter, currBlockVector, equalVectorLength);
    currTreeDepth = currBlockVector.size() - 1u;
    ocpiDebug("createTreeSeq currTreeDepth: %zd", currTreeDepth);
  ...
  ocpiDebug("Ending currTreeDepth: %zd", currTreeDepth);
...
//////////
std::vector<std::string> OcpigrObj::
...
//////////
size_t OcpigrObj::
compareBlockVectors(std::vector<std::string> &curr, std::vector<std::string> &prev) {
    // Determine smallest block between prev and current
    
    size_t
      currBlockSize = curr.size(),
      prevBlockSize = prev.size(),
      smallestBlockSize = (currBlockSize < prevBlockSize) ? currBlockSize : prevBlockSize;
    ...
    while (ndx < smallestBlockSize) {
      if (curr.at(ndx) != prev.at(ndx)) {
        ocpiDebug("curr and prev block vectors NOT equal at ndx: %zd", ndx);
        break;
      }
      ocpiDebug("curr and prev block vectors equal at ndx: %zd", ndx);
      ndx++;
    }
    return ndx;
}
...
//////////
void OcpigrObj::
createTreeSeq(YAML::Emitter &emitter, std::vector<std::string> &blockVector, size_t &startingDepth) {
  // Determine total sequence depth
  size_t totalSeqDepth = blockVector.size() - 1;
  ocpiDebug("createTreeSeq totalSeqDepth: %zd", totalSeqDepth);
  ocpiDebug("createTreeSeq startingDepth: %zd", startingDepth);
  
  for (size_t depth = startingDepth; depth < totalSeqDepth; depth++) {
    // Setup map for given depth
    emitter << YAML::BeginMap;

    // Insert sequence name and create sequence
    ocpiDebug("createTreeSeq current depth: %zd", depth+1);
...
//////////
void OcpigrObj::
endTreeSeq(YAML::Emitter &emitter, size_t &closeDepth) {
  ocpiDebug("endTreeSeq closeDepth: %zd", closeDepth);

  for (size_t ndx = 0; ndx < closeDepth; ndx++) {
    ocpiDebug("Closing sequence and map number: %zd", ndx+1);
    emitter << YAML::EndSeq;
    emitter << YAML::EndMap;
  }
}
...
// Output: "raw" - raw is the catch all type in GRC yaml blocks
//////////
std::string OcpigrObj::
stringToColorHash(std::string str) {
  uint32_t hash = 0;
