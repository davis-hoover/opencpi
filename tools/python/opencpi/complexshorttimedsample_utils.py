...
"""
ComplexShortTimedSample Utils

Commonly used functions and definitions used in the generation and verification
unit test scripts of workers using the ComplexShortTimedSample protocol
...
# Declare ComplexShortTimedSample opcode encoding
SAMPLE_OPCODE           = 0
TIME_OPCODE             = 1
SAMPLE_INTERVAL_OPCODE  = 2
FLUSH_OPCODE            = 3
    ...
    for i in range(num_cycles):
        ...
        while a < len(data):
            if len(data) - a < samples_per_message:
                utu.add_msg(f, SAMPLE_OPCODE, data[a:len(data)])
            else:
                utu.add_msg(f, SAMPLE_OPCODE, data[a:a+samples_per_message])
...
def parse_samples_data_from_msgs_in_file(file_to_be_parsed):
    ...
    for i in range(0,len(msg)):
        if(msg[i][utu.MESSAGE_OPCODE]==SAMPLE_OPCODE):
