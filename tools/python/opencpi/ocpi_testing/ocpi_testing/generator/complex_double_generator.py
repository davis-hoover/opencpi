...
# Class for generating complex double floating point number sample input data
#
...
# Default value is the same as for a real (not complex) double values
from .double_generator import DoubleGeneratorDefaults


class ComplexDoubleGeneratorDefaults:
    ...
    MESSAGE_SIZE_LONGEST = 1024


class ComplexDoubleGenerator(base_generator.BaseGenerator):
    """ Complex double protocol test data generator
    """

    def __init__(self):
        """ Initialise complex double generator class

        Defines the default values for the variables that control the values
        and size of messages that are generated.

        Returns:
            An initialised ComplexDoubleGenerator instance.
        """
        super().__init__()
        ...
        self.MESSAGE_SIZE_LONGEST = \
            ComplexDoubleGeneratorDefaults.MESSAGE_SIZE_LONGEST
    ...
    def sample(self, seed, subcase):
        ...
        if subcase == "all_zero":
            ...
            return [
                {"opcode": "sample",
                 "data": [complex(self.DOUBLE_MAXIMUM, self.DOUBLE_MAXIMUM)] *
                 self.SAMPLE_DATA_LENGTH}]

        elif subcase == "all_minimum":
            return [
                {"opcode": "sample",
                 "data": [complex(self.DOUBLE_MINIMUM, self.DOUBLE_MINIMUM)] *
                 self.SAMPLE_DATA_LENGTH}]
            ...
            for index in range(self.SAMPLE_DATA_LENGTH):
                data[index] = complex(
                    0.0,
                    random.uniform(self.DOUBLE_MINIMUM, self.DOUBLE_MAXIMUM))
            return [{"opcode": "sample", "data": data}]
            ...
            data = [complex(0.0, 0.0)] * self.SAMPLE_DATA_LENGTH
            for index in range(self.SAMPLE_DATA_LENGTH):
                near_random_limit = (self.DOUBLE_MAXIMUM -
                                     self.SAMPLE_NEAR_RANGE)
                data[index] = complex(
                    random.uniform(near_random_limit, self.DOUBLE_MAXIMUM),
                    random.uniform(near_random_limit, self.DOUBLE_MAXIMUM))
            return [{"opcode": "sample", "data": data}]
            ...
            data = [complex(0.0, 0.0)] * self.SAMPLE_DATA_LENGTH
            for index in range(self.SAMPLE_DATA_LENGTH):
                near_random_limit = (self.DOUBLE_MINIMUM +
                                     self.SAMPLE_NEAR_RANGE)
                data[index] = complex(
                    random.uniform(self.DOUBLE_MINIMUM, near_random_limit),
                    random.uniform(self.DOUBLE_MINIMUM, near_random_limit))
            return [{"opcode": "sample", "data": data}]
    ...
    def _get_sample_values(self, number_of_samples=None):
        """ Generate a random sample of complex double floating point numbers
