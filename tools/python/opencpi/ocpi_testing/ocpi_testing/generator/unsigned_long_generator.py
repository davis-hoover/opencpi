...
# Class for generating unsigned long sample input data
#
...
class UnsignedLongGeneratorDefaults:
    ...
    #: Minimum value an unsigned long number allows.
    UNSIGNED_LONG_MINIMUM = 0

    #: Maximum value an unsigned long number allows.
    UNSIGNED_LONG_MAXIMUM = (2**32) - 1

    #: Mid point of the range of values supported by an unsigned long.
    UNSIGNED_LONG_MID = UNSIGNED_LONG_MAXIMUM // 2
    ...
    TYPICAL_AMPLITUDE_MEAN = UNSIGNED_LONG_MAXIMUM // 2

    #: Width of the Gaussian probability distribution function of the random
    #: number generator used to set the amplitude of the sinusoidal waves that
    #: are combined in a sample message for the typical test case.
    TYPICAL_AMPLITUDE_DISTRIBUTION_WIDTH = UNSIGNED_LONG_MAXIMUM // 4
...
class UnsignedLongGenerator(base_generator.BaseGenerator):
    ...
    def __init__(self):
        ...
        self.MESSAGE_SIZE_LONGEST = \
            UnsignedLongGeneratorDefaults.MESSAGE_SIZE_LONGEST
        self.UNSIGNED_LONG_MINIMUM = \
            UnsignedLongGeneratorDefaults.UNSIGNED_LONG_MINIMUM
        self.UNSIGNED_LONG_MAXIMUM = \
            UnsignedLongGeneratorDefaults.UNSIGNED_LONG_MAXIMUM
        self.UNSIGNED_LONG_MID = \
            UnsignedLongGeneratorDefaults.UNSIGNED_LONG_MID
        self.TYPICAL_AMPLITUDE_MEAN = \
            UnsignedLongGeneratorDefaults.TYPICAL_AMPLITUDE_MEAN
        self.TYPICAL_AMPLITUDE_DISTRIBUTION_WIDTH = \
            UnsignedLongGeneratorDefaults.TYPICAL_AMPLITUDE_DISTRIBUTION_WIDTH
        self.TYPICAL_MAXIMUM_AMPLITUDE = \
            UnsignedLongGeneratorDefaults.TYPICAL_MAXIMUM_AMPLITUDE
    ...
    def typical(self, seed, subcase):
        ...
        for index in range(len(data)):
            ...
            data[index] = int(data[index] + self.UNSIGNED_LONG_MID)
    ...
    def sample(self, seed, subcase):
        ...
        if subcase == "all_zero":
            ...
            return [
                {"opcode": "sample",
                 "data": [self.UNSIGNED_LONG_MAXIMUM] *
            ...
            for index in range(self.SAMPLE_DATA_LENGTH):
                data[index] = random.randint(
                    self.UNSIGNED_LONG_MAXIMUM - self.SAMPLE_NEAR_RANGE,
                    self.UNSIGNED_LONG_MAXIMUM)
            return [{"opcode": "sample", "data": data}]
    ...
    def _full_scale_random_sample_values(self, number_of_samples=None):
        """ Generate a random sample of unsigned longs
        ...
        for index in range(number_of_samples):
            data[index] = random.randint(self.UNSIGNED_LONG_MINIMUM,
                                         self.UNSIGNED_LONG_MAXIMUM)
        return data
    ...
    def _get_sample_values(self, number_of_samples=None):
        ...
        unsigned longs can represent, this range size is set by
        ...
        limited_range_min = round(
            self.LIMITED_SCALE_FACTOR * self.UNSIGNED_LONG_MINIMUM)
        limited_range_max = round(
            self.LIMITED_SCALE_FACTOR * self.UNSIGNED_LONG_MAXIMUM)
