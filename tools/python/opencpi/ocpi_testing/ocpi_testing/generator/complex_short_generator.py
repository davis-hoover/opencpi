...
# Class for generating complex short sample input data
#
...
# Default value is the same as for a real (not complex) short values
from .short_generator import ShortGeneratorDefaults


class ComplexShortGeneratorDefaults:
...
class ComplexShortGenerator(base_generator.BaseGenerator):
    """ Complex short protocol test data generator
    """

    def __init__(self):
        """ Initialise complex short generator class
        ...
        self.MESSAGE_SIZE_LONGEST = \
            ComplexShortGeneratorDefaults.MESSAGE_SIZE_LONGEST
    ...
    def sample(self, seed, subcase):
        ...
        if subcase == "all_zero":
            ...
            return [
                {"opcode": "sample",
                 "data": [complex(self.SHORT_MAXIMUM, self.SHORT_MAXIMUM)] *
                 self.SAMPLE_DATA_LENGTH}]

        elif subcase == "all_minimum":
            return [
                {"opcode": "sample",
                 "data": [complex(self.SHORT_MINIMUM, self.SHORT_MINIMUM)] *
                 self.SAMPLE_DATA_LENGTH}]
            ...
            for index in range(self.SAMPLE_DATA_LENGTH):
                data[index] = complex(
                    0, random.randint(self.SHORT_MINIMUM, self.SHORT_MAXIMUM))
            return [{"opcode": "sample", "data": data}]
            ...
            for index in range(self.SAMPLE_DATA_LENGTH):
                data[index] = complex(
                    random.randint(self.SHORT_MINIMUM, self.SHORT_MAXIMUM), 0)
            return [{"opcode": "sample", "data": data}]

        elif subcase == "large_positive":
            data = [complex(0, 0)] * self.SAMPLE_DATA_LENGTH
            for index in range(self.SAMPLE_DATA_LENGTH):
                near_random_limit = self.SHORT_MAXIMUM - self.SAMPLE_NEAR_RANGE
                data[index] = complex(
                    random.randint(near_random_limit, self.SHORT_MAXIMUM),
                    random.randint(near_random_limit, self.SHORT_MAXIMUM))
            return [{"opcode": "sample", "data": data}]
            ...
            data = [complex(0, 0)] * self.SAMPLE_DATA_LENGTH
            for index in range(self.SAMPLE_DATA_LENGTH):
                near_random_limit = self.SHORT_MINIMUM + self.SAMPLE_NEAR_RANGE
                data[index] = complex(
                    random.randint(self.SHORT_MINIMUM, near_random_limit),
                    random.randint(self.SHORT_MINIMUM, near_random_limit))
            return [{"opcode": "sample", "data": data}]
    ...
    def _full_scale_random_sample_values(self, number_of_samples=None):
        ...
        for index in range(number_of_samples):
            data[index] = complex(
                random.randint(self.SHORT_MINIMUM, self.SHORT_MAXIMUM),
                random.randint(self.SHORT_MINIMUM, self.SHORT_MAXIMUM))
        return data
