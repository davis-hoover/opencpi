...
class LongGeneratorDefaults:
    ...
    #: Minimum value a long number allows.
    LONG_MINIMUM = -(2**31)
    ...
    TYPICAL_AMPLITUDE_MEAN = LONG_MAXIMUM // 2

    #: Width of the Gaussian probability distribution function of the random
    #: number generator used to set the amplitude of the sinusoidal waves that
    #: are combined in a sample message for the typical test case.
    TYPICAL_AMPLITUDE_DISTRIBUTION_WIDTH = LONG_MAXIMUM // 4
    ...
    TYPICAL_MAXIMUM_AMPLITUDE = (
        LONG_MAXIMUM * base_generator.GeneratorDefaults.LIMITED_SCALE_FACTOR)


class LongGenerator(base_generator.BaseGenerator):
    """ Long protocol test data generator
    """

    def __init__(self):
        """ Initialise long generator class
    ...
    def sample(self, seed, subcase):
        ...
        if subcase == "all_zero":
            ...
            return [{"opcode": "sample",
                     "data": [self.LONG_MAXIMUM] * self.SAMPLE_DATA_LENGTH}]

        elif subcase == "all_minimum":
            return [{"opcode": "sample",
                     "data": [self.LONG_MINIMUM] * self.SAMPLE_DATA_LENGTH}]
    ...
    def _full_scale_random_sample_values(self, number_of_samples=None):
        ...
        for index in range(number_of_samples):
            data[index] = random.randint(self.LONG_MINIMUM, self.LONG_MAXIMUM)
        return data
