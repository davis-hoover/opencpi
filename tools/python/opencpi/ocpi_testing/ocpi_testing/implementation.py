...
class Implementation(abc.ABC):
    ...
    def process_messages(self, *inputs, **named_inputs):
        ...
        if len(self.input_ports) == 1:
            ...
            while any([next_message_index[index] < len(inputs[index])
        ...
        # Call the end of files method and append any output generated by it.
        resulting_messages = self.end_of_files()
        for port_index, port_data in enumerate(resulting_messages):
            if len(port_data):
                output_messages[port_index] = \
                    output_messages[port_index] + port_data
    ...
    def end_of_files(self):
        """Generate any additional messages when all input files have ended.

        Set with default behaviour here, however if a different behaviour is
        needed this should be overridden by the specific implementation. For
        example, a specific implementation may want to complete and output
        a partial frame, or ensure a footer is present.
        ...
        Returns:
            A named tuple where each item in the tuple relates to each output
                ...
                and "data".
        """
        # Default to no extra messages on reaching the end of all files
