...
from ocpi_testing.generator import ComplexShortGenerator
...
class TestComplexShortGenerator(unittest.TestCase):
    def setUp(self):
        self.test_generator = ComplexShortGenerator()
        self.seed = 347

    def assertComplexShort(self, value):
        self.assertIsInstance(value, complex)
        self.assertGreaterEqual(value.real, SHORT_MIN)
        self.assertLessEqual(value.real, SHORT_MAX)
        self.assertGreaterEqual(value.imag, SHORT_MIN)
        self.assertLessEqual(value.imag, SHORT_MAX)
    ...
    def test_typical_single_wave(self):
        ...
        for index, (test_value, reference_value) in enumerate(
            ...
            self.assertComplexShort(test_value)
    ...
    def test_sample_all_maximum_subcase(self):
        ...
        for value in messages[0]["data"]:
            self.assertComplexShort(value)
            self.assertEqual(value, complex(SHORT_MAX, SHORT_MAX))
    ...
    def test_sample_all_minimum_subcase(self):
        ...
        for value in messages[0]["data"]:
            self.assertComplexShort(value)
            self.assertEqual(value, complex(SHORT_MIN, SHORT_MIN))
