...
# Testing of code in unsigned_long_long_generator.py
#
...
from opencpi.ocpi_testing.ocpi_testing.generator import UnsignedLongLongGenerator


UNSIGNED_LONG_LONG_MIN = 0
UNSIGNED_LONG_LONG_MAX = (2**64) - 1


class TestUnsignedLongLongGenerator(unittest.TestCase):
    def setUp(self):
        self.test_generator = UnsignedLongLongGenerator()
        self.seed = 53485

    def assertUnsignedLongLong(self, value):
        self.assertIsInstance(value, int)
        self.assertGreaterEqual(value, UNSIGNED_LONG_LONG_MIN)
        self.assertLessEqual(value, UNSIGNED_LONG_LONG_MAX)
    ...
    def test_typical(self):
        ...
        for value in messages[0]["data"]:
            self.assertUnsignedLongLong(value)
    ...
    def test_typical_single_wave(self):
        frequency = 0.1
        amplitude = 50
        offset = UNSIGNED_LONG_LONG_MAX / 2
        ...
        for index, (test_value, reference_value) in enumerate(
            ...
            self.assertUnsignedLongLong(test_value)
    ...
    def test_sample_all_maximum_subcase(self):
        ...
        for value in messages[0]["data"]:
            self.assertUnsignedLongLong(value)
            self.assertEqual(value, UNSIGNED_LONG_LONG_MAX)
    ...
    def test_sample_large_positive_subcase(self):
        ...
        self.assertEqual(len(messages[0]["data"]),
                         self.test_generator.SAMPLE_DATA_LENGTH)
        min_expected_value = UNSIGNED_LONG_LONG_MAX - self.test_generator.SAMPLE_NEAR_RANGE
        for value in messages[0]["data"]:
            self.assertUnsignedLongLong(value)
