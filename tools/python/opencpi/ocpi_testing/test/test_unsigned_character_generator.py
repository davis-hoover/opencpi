...
# Testing of code in unsigned_character_generator.py
#
...
from ocpi_testing.generator import UnsignedCharacterGenerator


UNSIGNED_CHARACTER_MIN = 0
UNSIGNED_CHARACTER_MAX = (2**8) - 1


class TestUnsignedCharacterGenerator(unittest.TestCase):
    def setUp(self):
        self.test_generator = UnsignedCharacterGenerator()
        self.seed = 894

    def assertUnsignedCharacter(self, value):
        self.assertIsInstance(value, int)
        self.assertGreaterEqual(value, UNSIGNED_CHARACTER_MIN)
        self.assertLessEqual(value, UNSIGNED_CHARACTER_MAX)
    ...
    def test_typical(self):
        ...
        for value in messages[0]["data"]:
            self.assertUnsignedCharacter(value)
    ...
    def test_typical_single_wave(self):
        frequency = 0.1
        amplitude = 50
        offset = UNSIGNED_CHARACTER_MAX / 2
        ...
        # relation to whole supported range of unsigned characters)
        self.assertEqual(len(messages), 1)
        self.assertEqual(messages[0]["opcode"], "sample")
        self.assertEqual(len(messages[0]["data"]),
                         self.test_generator.SAMPLE_DATA_LENGTH)
        for index, (test_value, reference_value) in enumerate(
            ...
            self.assertUnsignedCharacter(test_value)
    ...
    def test_sample_all_maximum_subcase(self):
        ...
        for value in messages[0]["data"]:
            self.assertUnsignedCharacter(value)
            self.assertEqual(value, UNSIGNED_CHARACTER_MAX)
    ...
    def test_sample_large_positive_subcase(self):
        ...
        self.assertEqual(len(messages[0]["data"]),
                         self.test_generator.SAMPLE_DATA_LENGTH)
        min_expected_value = UNSIGNED_CHARACTER_MAX - \
            self.test_generator.SAMPLE_NEAR_RANGE
        for value in messages[0]["data"]:
            self.assertUnsignedCharacter(value)
