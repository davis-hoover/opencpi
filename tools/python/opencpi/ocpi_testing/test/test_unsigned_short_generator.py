...
# Testing of code in unsigned_short_generator.py
#
...
from ocpi_testing.generator import UnsignedShortGenerator


UNSIGNED_SHORT_MIN = 0
UNSIGNED_SHORT_MAX = (2**16) - 1


class TestUnsignedShortGenerator(unittest.TestCase):
    def setUp(self):
        self.test_generator = UnsignedShortGenerator()
        self.seed = 31794

    def assertUnsignedShort(self, value):
        self.assertIsInstance(value, int)
        self.assertGreaterEqual(value, UNSIGNED_SHORT_MIN)
        self.assertLessEqual(value, UNSIGNED_SHORT_MAX)

    def test_typical(self):
        messages = self.test_generator.generate(self.seed, "typical", None,
                                                "01", "02")
        ...
        for value in messages[0]["data"]:
            self.assertUnsignedShort(value)

    def test_typical_same_seed_same_messages(self):
        messages_1 = self.test_generator.generate(self.seed, "typical", None,
                                                  "01", "02")
        messages_2 = self.test_generator.generate(self.seed, "typical", None,
                                                  "01", "02")
    ...
    def test_typical_single_wave(self):
        ...
        # relation to whole supported range of unsigned shorts)
        self.assertEqual(len(messages), 1)
        self.assertEqual(messages[0]["opcode"], "sample")
        self.assertEqual(len(messages[0]["data"]),
                         self.test_generator.SAMPLE_DATA_LENGTH)
        for index, (test_value, reference_value) in enumerate(
            ...
            self.assertUnsignedShort(test_value)
    ...
    def test_sample_all_maximum_subcase(self):
        ...
        for value in messages[0]["data"]:
            self.assertUnsignedShort(value)
            self.assertEqual(value, UNSIGNED_SHORT_MAX)
    ...
    def test_sample_large_positive_subcase(self):
        ...
        self.assertEqual(len(messages[0]["data"]),
                         self.test_generator.SAMPLE_DATA_LENGTH)
        min_expected_value = UNSIGNED_SHORT_MAX - self.test_generator.SAMPLE_NEAR_RANGE
        for value in messages[0]["data"]:
            self.assertUnsignedShort(value)
