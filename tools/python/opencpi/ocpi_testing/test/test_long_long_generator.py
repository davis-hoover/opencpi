...
# Testing of code in long_long_generator.py
#
...
from opencpi.ocpi_testing.ocpi_testing.generator import LongLongGenerator
...
class TestLongLongGenerator(unittest.TestCase):
    ...
    def assertLongLong(self, value):
        self.assertIsInstance(value, int)
        self.assertGreaterEqual(value, LONG_LONG_MIN)
        self.assertLessEqual(value, LONG_LONG_MAX)
    ...
    def test_typical_single_wave(self):
        ...
        for index, (test_value, reference_value) in enumerate(
            ...
            self.assertLongLong(test_value)
            # Ensure the error between values is less than 5
            self.assertLess(abs(test_value - reference_value), 5)
    ...
    def test_sample_all_maximum_subcase(self):
        ...
        for value in messages[0]["data"]:
            self.assertLongLong(value)
            self.assertEqual(value, LONG_LONG_MAX)
    ...
    def test_sample_all_minimum_subcase(self):
        ...
        for value in messages[0]["data"]:
            self.assertLongLong(value)
            self.assertEqual(value, LONG_LONG_MIN)
    ...
    def test_sample_large_positive_subcase(self):
        ...
        self.assertEqual(len(messages[0]["data"]),
                         self.test_generator.SAMPLE_DATA_LENGTH)
        min_expected_value = LONG_LONG_MAX - self.test_generator.SAMPLE_NEAR_RANGE
        for value in messages[0]["data"]:
            self.assertLongLong(value)
    ...
    def test_sample_large_negative_subcase(self):
        ...
        self.assertEqual(len(messages[0]["data"]),
                         self.test_generator.SAMPLE_DATA_LENGTH)
        max_expected_value = LONG_LONG_MIN + self.test_generator.SAMPLE_NEAR_RANGE
        for value in messages[0]["data"]:
            self.assertLongLong(value)
