...
set_tool_platform() { 
  if test "$OCPI_TOOL_PLATFORM" == ""; then
    ...
    if test -f release; then  # checks to see what is being ran
      ...
      echo Error: OCPI_TOOL_PLATFORM not set properly, $OCPI_LOCAL_DIR/release not found
      break  
    fi
  fi
}
...
set_time() {
  if test "$1" != -; then
    ...
    OPTS=""
    BUSYBOX_PATH="$OCPI_LOCAL_DIR/$OCPI_TOOL_PLATFORM/bin"
    TIMEOUT=5
    MSG="Succeeded in setting the time from $OCPI_LOCAL_DIR/ntp.conf"
    if [ ! -e $OCPI_LOCAL_DIR/ntp.conf ]; then
      OPTS="-p $1"
